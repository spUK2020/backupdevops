    ...
    static async Task Main(string[] args)
    {
        using (var client = new HttpClient())
        {
            ...
            foreach (var project in projects.value)
            {
                ...
                // Get a list of repositories for the project
                var reposResponse = await client.GetAsync($"{organizationUrl}/{projectName}/_apis/git/repositories?api-version=6.0");
                reposResponse.EnsureSuccessStatusCode();
                var repos = await reposResponse.Content.ReadAsAsync<dynamic>();
